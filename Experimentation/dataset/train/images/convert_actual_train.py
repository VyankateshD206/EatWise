...
# Load COCO JSON file
with open('C:/Users/honpa/OneDrive/Desktop/Programming/DC(sem 4)/Research papers/NutrifyAI using EDAMAM implementation/train/_annotations.coco.json', 'r') as f:
...
df.to_excel('annotations_train.xlsx', index=False)

df = pd.read_excel('annotations_train.xlsx')
df['CaloriesPerServing'] = df['label'].map(calorie_dict)

df.to_excel('annotations_train.xlsx', index=False)

print("Calorie mapping completed and saved to 'annotations_train.xlsx'.")
